...
function BlurShieldPage(){const host=useRef<HTMLDivElement>(null);const[root,setRoot]=useState<ShadowRoot|null>(null);useEffect(()=>{if(!host.current)return;const shadow=host.current.shadowRoot||host.current.attachShadow({mode:'open'});const style=document.createElement('style');style.textContent=`:host{display:block;color-scheme:light}.app-shell{margin:0;background:#fbfaf7;color:#1b1930;font-family:Arial,Helvetica,sans-serif}${blurShieldStyles.replace(':root',':host').replaceAll('body{','.app-shell{').replace('html{scroll-behavior:smooth}','')}`;shadow.appendChild(style);setRoot(shadow);return()=>{setRoot(null);shadow.replaceChildren()}},[]);return <main className="blurshield-route" ref={host}>{root&&createPortal(<Suspense fallback={<div className="tool-loading">Loading photo editor…</div>}><BlurShield/></Suspense>,root)}</main>}
