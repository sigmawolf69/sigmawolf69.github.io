...
function Carousel({title,href,items,navigate}:{title:string;href:string;items:Video[];navigate:(href:string)=>void}){
  ...
  return <section className="home-section"><div className="carousel-heading"><Link href={href} navigate={navigate}>{title} <span>●</span></Link><div><Button variant="outline" size="icon" onClick={()=>move(-1)}>‹</Button><Button variant="outline" size="icon" onClick={()=>move(1)}>›</Button></div></div><div className="carousel-rail" ref={rail}>{items.map(v=><VideoCard key={v.id} video={v} navigate={navigate}/>)}</div></section>
}
...
function DirectoryCarousel({title,href,items,navigate}:{title:string;href:string;items:{name:string;video:Video;count:number}[];navigate:(href:string)=>void}){
  const rail=useRef<HTMLDivElement>(null);return <section className="home-section"><div className="carousel-heading"><Link href={href} navigate={navigate}>{title} <span>●</span></Link><div><Button variant="outline" size="icon" onClick={()=>rail.current?.scrollBy({left:-700,behavior:'smooth'})}>‹</Button><Button variant="outline" size="icon" onClick={()=>rail.current?.scrollBy({left:700,behavior:'smooth'})}>›</Button></div></div><div className="carousel-rail directory-rail" ref={rail}>{items.map(x=><Link key={x.name} href={`${href}?${href.includes('categor')?'tag':'brand'}=${encodeURIComponent(x.name)}`} navigate={navigate} className="directory-tile" style={art(x.video)}><strong>{x.name}</strong><small>{x.count} releases</small></Link>)}</div></section>
}
...
function Home({navigate}:{navigate:(href:string)=>void}){
  ...
  const tags=useMemo(()=>[...new Set(videos.flatMap(v=>v.tags||[]))].slice(0,18).map(name=>({name,count:videos.filter(v=>v.tags?.includes(name)).length,video:videos.find(v=>v.tags?.includes(name))!})),[])
  const brands=useMemo(()=>[...new Set(videos.map(v=>v.brand).filter(Boolean))].slice(0,18).map(name=>({name,count:videos.filter(v=>v.brand===name).length,video:videos.find(v=>v.brand===name)!})),[])
  const peak=trending[0];return <main className="home"><section className="welcome"><p>Welcome back</p><h1>What are we watching today?</h1></section>{peak&&<section className="peak" style={{backgroundImage:`linear-gradient(90deg,#101010 15%,#101010aa),url("${mediaUrl(peak.backdrop||peak.cover)}")`}}><div><span>Peak of the day</span><h2>{peak.title}</h2><Link href={`/search?brand=${encodeURIComponent(peak.brand)}`} navigate={navigate} className="peak-brand">{peak.brand}</Link><p>{peak.description||`${peak.title} episode ${peak.ep}, available in ${peak.quality}.`}</p><Link href={`/video/${peak.slug}`} navigate={navigate}>▶ Watch now</Link></div><VideoCard video={peak} navigate={navigate}/></section>}
