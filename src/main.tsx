...
declare global{interface Window{dataLayer:unknown[];gtag?:(...args:unknown[])=>void;AdProvider?:unknown[]}}
...
function useRoute(){
  const read=():Route=>{const current=new URL(window.location.href);const fallback=current.searchParams.get('__route');if(fallback){history.replaceState({},'',fallback);return read()}const actual=current.pathname.replace(/\/+$/,'')||'/';const noAds=actual==='/noads'||actual.startsWith('/noads/');const path=noAds?(actual.slice(6)||'/'):actual;return{path,params:current.searchParams,noAds}}
  ...
  return[route,(href:string)=>{const target=route.noAds&&href.startsWith('/')&&!href.startsWith('/noads')?`/noads${href==='/'?'':href}`:href;history.pushState({},'',target);setRoute(read());window.scrollTo({top:0})}] as const
}
...
function Link({href,navigate,className,style,children}:{href:string;navigate:(href:string)=>void;className?:string;style?:React.CSSProperties;children:React.ReactNode}){
  const noAds=location.pathname==='/noads'||location.pathname.startsWith('/noads/');const target=noAds&&href.startsWith('/')&&!href.startsWith('/noads')?`/noads${href==='/'?'':href}`:href
  return <a href={target} className={className} style={style} onClick={e=>{if(!e.ctrlKey&&!e.metaKey&&!e.shiftKey&&!e.altKey){e.preventDefault();navigate(href)}}}>{children}</a>
}
...
function Header({route,navigate}:{route:Route;navigate:(href:string)=>void}){
  ...
  return <><header className="header"><Link href="/" navigate={go} className="brand"><span className="brand-mark">T</span><span className="brand-copy"><strong>Titties</strong><small>Discover in HD</small></span></Link>
    <nav className={open?'nav open':'nav'}><div className="mobile-nav-title"><span>Explore</span><Button variant="ghost" size="icon-sm" onClick={()=>setOpen(false)} aria-label="Close menu"><X/></Button></div><Link href="/" navigate={go} className={active('/')?'active':''}>Home</Link><Link href="/search" navigate={go} className={active('/search')?'active':''}>Search</Link><Link href="/categories" navigate={go} className={active('/categories')?'active':''}>Categories</Link><Link href="/brands" navigate={go} className={active('/brands')?'active':''}>Brands</Link><Link href="/random" navigate={go} className={active('/random')?'active':''}>Random</Link><Link href="/blurshield" navigate={go} className={active('/blurshield')?'active':''}>Photo Blur</Link></nav>
...
function Analytics({route}:{route:Route}){useEffect(()=>{window.gtag?.('event','page_view',{page_title:document.title,page_location:window.location.href,page_path:window.location.pathname+window.location.search})},[route.path,route.noAds,route.params.toString()]);return null}

function Seo({route}:{route:Route}){useEffect(()=>{const editor=route.path==='/blurshield',slug=route.path.startsWith('/video/')?decodeURIComponent(route.path.slice(7)):'',video=slug?videos.find(v=>v.slug===slug||v.id===slug):undefined,tag=route.params.get('tag'),brand=route.params.get('brand');let title='Titties — HD video library',description='Browse videos by title, category, brand, popularity, and release date.';if(editor){title='BlurShield — Free private photo blur and redaction tool';description='Blur, pixelate, or cover sensitive areas in photos privately in your browser. Batch edit images and download them without uploading files.'}else if(video){title=`${video.title} — Watch in ${video.quality||'HD'}`;description=(video.description||`Watch ${video.title} from ${video.brand} in ${video.quality||'HD'}.`).slice(0,160)}else if(route.path==='/categories'){title='Video Categories — Browse the complete library';description='Explore the complete collection of video categories and discover releases by tag.'}else if(route.path==='/brands'){title='Video Brands — Browse studios and creators';description='Explore every available brand and find its latest and most popular releases.'}else if(route.path==='/search'){title=`${brand||tag||route.params.get('q')||'Search'} — Video results`;description=`Browse matching videos${brand?` from ${brand}`:''}${tag?` in ${tag}`:''}, with filters and pagination.`}const canonicalUrl=`https://hentaititties.com${route.path==='/'?'':route.path}`;const image=video?mediaUrl(video.backdrop||video.cover):'https://hentaititties.com/favicon.ico';document.title=title;const set=(selector:string,attribute:string,value:string)=>{const node=document.querySelector<HTMLMetaElement>(selector);if(node)node.setAttribute(attribute,value)};set('meta[name="description"]','content',description);set('meta[property="og:type"]','content',video?'video.other':'website');set('meta[property="og:title"]','content',title);set('meta[property="og:description"]','content',description);set('meta[property="og:url"]','content',canonicalUrl);set('meta[property="og:image"]','content',image);set('meta[name="twitter:title"]','content',title);set('meta[name="twitter:description"]','content',description);set('meta[name="twitter:image"]','content',image);const canonical=document.querySelector<HTMLLinkElement>('link[rel="canonical"]');if(canonical)canonical.href=canonicalUrl},[route.path,route.params.toString()]);return null}

function BlurShieldPage(){const host=useRef<HTMLDivElement>(null);const[root,setRoot]=useState<ShadowRoot|null>(null);useEffect(()=>{if(!host.current)return;const element=host.current,setTheme=()=>{element.dataset.theme=document.documentElement.classList.contains('dark')?'dark':'light'};setTheme();const observer=new MutationObserver(setTheme);observer.observe(document.documentElement,{attributes:true,attributeFilter:['class']});const shadow=element.shadowRoot||element.attachShadow({mode:'open'});const style=document.createElement('style');style.textContent=`:host{display:block;color-scheme:light}.app-shell{margin:0;background:#fbfaf7;color:#1b1930;font-family:Arial,Helvetica,sans-serif}${blurShieldStyles.replace(':root',':host').replaceAll('body{','.app-shell{').replace('html{scroll-behavior:smooth}','')}`;shadow.appendChild(style);setRoot(shadow);return()=>{observer.disconnect();setRoot(null);shadow.replaceChildren()}},[]);return <main className="blurshield-route" ref={host}>{root&&createPortal(<Suspense fallback={<div className="tool-loading">Loading photo editor…</div>}><BlurShield/></Suspense>,root)}</main>}

function Ads({disabled,routeKey}:{disabled:boolean;routeKey:string}){
  useEffect(()=>{if(disabled)return;const provider=document.createElement('script');provider.async=true;provider.type='application/javascript';provider.src='https://a.magsrv.com/ad-provider.js';provider.dataset.siteAd='provider';provider.dataset.routeKey=routeKey;document.body.appendChild(provider);window.AdProvider=window.AdProvider||[];window.AdProvider.push({serve:{}});const interstitial=document.createElement('script');interstitial.async=true;interstitial.type='application/javascript';interstitial.src='https://a.pemsrv.com/fp-interstitial.js';interstitial.dataset.idzone='5108474';interstitial.dataset.ad_frequency_count='1';interstitial.dataset.ad_frequency_period='5';interstitial.dataset.type='desktop';interstitial.dataset.browser_settings='1';interstitial.dataset.ad_trigger_method='3';interstitial.dataset.siteAd='interstitial';interstitial.dataset.routeKey=routeKey;document.body.appendChild(interstitial);return()=>{provider.remove();interstitial.remove()}},[disabled,routeKey])
  if(disabled)return null
  return <aside className="ad-wrap" aria-label="Advertisement"><ins className="eas6a97888e ad-slot" data-zoneid="5108472" data-site-ad="display"/></aside>
}
...
function DirectoryCarousel({title,href,items,navigate}:{title:string;href:string;items:{name:string;video:Video;count:number}[];navigate:(href:string)=>void}){
  const rail=useRef<HTMLDivElement>(null);const filterKey=href.includes('categor')?'tag':'brand';return <section className="home-section"><div className="carousel-heading"><Link href={href} navigate={navigate}>{title} <span>●</span></Link><div className="section-actions"><Button asChild variant="ghost" size="sm" className="section-link"><Link href={href} navigate={navigate}>Explore <span>→</span></Link></Button><span className="carousel-buttons"><Button variant="outline" size="icon" onClick={()=>rail.current?.scrollBy({left:-700,behavior:'smooth'})} aria-label={`Previous ${title}`}>‹</Button><Button variant="outline" size="icon" onClick={()=>rail.current?.scrollBy({left:700,behavior:'smooth'})} aria-label={`Next ${title}`}>›</Button></span></div></div><div className="carousel-rail directory-rail" ref={rail}>{items.map(x=><Link key={x.name} href={`/search?${filterKey}=${encodeURIComponent(x.name)}`} navigate={navigate} className="directory-tile" style={art(x.video)}><strong>{x.name}</strong><small>{x.count} releases</small></Link>)}</div></section>
}
...
function Home({navigate}:{navigate:(href:string)=>void}){
  ...
  const peak=useMemo(()=>videos.length?videos[Math.floor(Math.random()*videos.length)]:undefined,[]);return <main className="home"><section className="welcome"><p>Welcome back</p><h1>What are we watching today?</h1></section>{peak&&<section className="peak"><span className="peak-backdrop" aria-hidden="true" style={{backgroundImage:`url("${mediaUrl(peak.backdrop||peak.cover)}")`}}/><div><span>Peak of the day</span><h2>{peak.title}</h2><Link href={`/search?brand=${encodeURIComponent(peak.brand)}`} navigate={navigate} className="peak-brand">{peak.brand}</Link><div className="video-description">{peak.description||`${peak.title} episode ${peak.ep}, available in ${peak.quality}.`}</div><Link href={`/video/${peak.slug}`} navigate={navigate}>▶ Watch now</Link></div><VideoCard video={peak} navigate={navigate}/></section>}
...
function SearchPage({route,navigate}:{route:Route;navigate:(href:string)=>void}){
  ...
  const href=(changes:Record<string,string|number>)=>{const p=new URLSearchParams(route.params);Object.entries(changes).forEach(([k,v])=>v?p.set(k,String(v)):p.delete(k));return`${route.noAds?'/noads':''}/search${p.size?`?${p}`:''}`}
